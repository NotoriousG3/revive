class NameManager {
    ...
    async GetNames() {
        let result = await this.#m_Api.GetNames();
        this.Names = result.data;
    }
    
    async Delete(id) {
        await this.#m_Api.DeleteName(id);
    }
    ...
    async Upload(fileSelector, callback) {
        ...
        let result = await this.#m_Api.ImportNames(uploadResponse.data);
    ...
    async Purge() {
        return await this.#m_Api.PurgeNames();
    }
}