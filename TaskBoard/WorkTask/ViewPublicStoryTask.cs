...
public class ViewPublicStoryTask: ActionWorkTask
{
    public ViewPublicStoryTask(WorkScheduler scheduler, WorkLogger logger, SnapchatActionRunner runner, SnapchatAccountManager accountManager, AccountTracker accountTracker, TargetManager targetManager, IServiceProvider serviceProvider): base(scheduler, logger, accountManager, runner, accountTracker, targetManager, serviceProvider)
    {
        CheckForMedia = false;
    }

    private async Task<WorkStatus> DoWork(WorkRequest work, ViewPublicStoryArguments arguments, SnapchatAccountModel account)
    {
        await Logger.LogDebug(work, "Starting Public View Story task", account);
        ...
        try
        {
            ...
            await Runner.ViewPublicStory(account, arguments.Username, proxyGroup, work.CancellationTokenSource.Token);
    ...
    public async Task Start(WorkRequest work, ViewPublicStoryArguments arguments, SnapchatActionsWorker worker, bool useArgumentsAccountToUse = false)
    {
