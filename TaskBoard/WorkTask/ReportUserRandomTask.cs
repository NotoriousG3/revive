...
public class ReportUserRandomTask: ActionWorkTask
{
    public ReportUserRandomTask(WorkScheduler scheduler, WorkLogger logger, SnapchatActionRunner runner,
        SnapchatAccountManager accountManager, AccountTracker accountTracker, TargetManager targetManager, IServiceProvider serviceProvider) : base(
        scheduler, logger, accountManager, runner, accountTracker, targetManager, serviceProvider)
    {
        CheckForMedia = false;
    }
    private async Task<WorkStatus> DoWork(WorkRequest work, ReportUserRandomArguments arguments, SnapchatAccountModel account)
    {
        await Logger.LogDebug(work, "Starting ReportUserRandom task", account);
        ...
        try
        {
            ...
            if (!await TryAddFriend(work, account, proxyGroup, arguments.Username))
            {
                await scheduler.FailWorkAccount(work, account);
                return WorkStatus.Error;
            }

            await Runner.ReportUserRandom(account, arguments.Username, proxyGroup, work.CancellationTokenSource.Token);
            await scheduler.UpdateWorkAddPass(work);
            await Logger.LogInformation(work, $"Report sent against {arguments.Username}!", account);
    ...
    public async Task Start(WorkRequest work, ReportUserRandomArguments arguments, SnapchatActionsWorker worker, bool useArgumentsAccountToUse = false)
    {
