...
public class ViewBusinessPublicStoryTask: ActionWorkTask
{
    public ViewBusinessPublicStoryTask(WorkScheduler scheduler, WorkLogger logger, SnapchatActionRunner runner, SnapchatAccountManager accountManager, AccountTracker accountTracker, TargetManager targetManager, IServiceProvider serviceProvider): base(scheduler, logger, accountManager, runner, accountTracker, targetManager, serviceProvider)
    {
        CheckForMedia = false;
    }

    private async Task<WorkStatus> DoWork(WorkRequest work, ViewBusinessPublicStoryArguments arguments, SnapchatAccountModel account)
    {
        if (work.CancellationTokenSource.IsCancellationRequested)
            return WorkStatus.Cancelled;

        await Logger.LogDebug(work, "Starting Public Business View Story task", account);
        ...
        try
        {
            ...
            await Runner.ViewBusinessPublicStory(account, arguments.Username, proxyGroup, work.CancellationTokenSource.Token);
            await scheduler.UpdateWorkAddPass(work);
            await Logger.LogInformation(work, $"Viewed {arguments.Username} story.", account);
    ...
    public async Task Start(WorkRequest work, ViewBusinessPublicStoryArguments arguments, SnapchatActionsWorker worker, bool useArgumentsAccountToUse = false)
    {
