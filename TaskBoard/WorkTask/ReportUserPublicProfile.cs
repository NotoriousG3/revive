...
public class ReportPublicProfileTask : ActionWorkTask
{
        ...
        SnapchatAccountModel account)
    {
        ...
        try
        {
            ...
            var result = TryAddFriend(work, account, proxyGroup, arguments.Username).GetAwaiter().GetResult();

            // We don't want to continue if the friend could not be added
            if (!result)
            {
            ...
            await Runner.ReportUserPublicProfileRandom(account, arguments.Username, proxyGroup, work.CancellationTokenSource.Token);
    ...
    public async Task Start(WorkRequest work, ReportUserPublicProfileRandomArguments arguments, SnapchatActionsWorker worker, bool useArgumentsAccountToUse = false)
    {
